          ...
          {jobsAgenciesData?.items?.map((jobAgency: JobAgency) => (
            ...
            >
              <div className="gap-2 items-center">
                <h3 className="font-semibold text-xl h-20 sm:text-6xl sm:font-medium capitalize bg-gradient-to-r from-purple-700 to-blue-700 text-transparent bg-clip-text">
                  {jobAgency.title}
                </h3>
              ...
              <Link href={`/job-details/${jobAgency.id}/${agency.slug}`}>
                <button className="text-gray-50 px-4 sm:px-8 py-2 rounded-full transition-all duration-200 hover:scale-110 bg-gradient-to-r from-blue-700 to-purple-700 ">
                  Aplicar
                </button>
              </Link>
            </div>
